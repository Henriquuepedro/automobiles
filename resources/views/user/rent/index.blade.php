...
@section('js')
    <script>
        ...
        const getOptionalsAutos = async () => {
            await $.get(`${window.location.origin}/ajax/aluguel/caracteristica/buscar`, function (optionals) {
...
@section('body')
    <div class="featured-car content-area-4">
        <div class="container">
            <div class="row widget mb-3">
                <div class="col-lg-12 col-md-12 d-flex">
                    <div class="form-search form-group col-lg-6 no-padding">
                        <label>Local de retirada:</label>
                        <select class="selectpicker search-fields">
                            <option value="">Selecione um local</option>
                            @foreach($places as $place)
                                <option value="{{ $place->id }}">{{ $place->address_public_place }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-search form-group col-lg-3 no-padding">
                        <label>Retirar Em:</label>
                        <input type="datetime-local" class="form-control border bg-white" placeholder="Data de Retirada"/>
                    </div>
                    <div class="form-search form-group col-lg-3 no-padding">
                        <label>Entregar Em:</label>
                        <input type="datetime-local" class="form-control border bg-white" placeholder="Data de Devolução"/>
                    </div>
                </div>
            </div>
            <div class="row">
